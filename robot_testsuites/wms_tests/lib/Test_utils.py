...
class Test_utils:
    ...
    def get_from_coumpound_job_all_nodes_ids(self,parent_jobid):
        ...
        self.log_info("Get all nodes ids for coumpound job: %s"%(parent_jobid))

        output=self.run_command("glite-wms-job-status -v 0 %s"%(parent_jobid)).split("\n")
        ...
        self.log_info("Node ids: %s"%(ids))

        return ids


    def get_target_ces(self,custom_requirements):
    ...
    def prepare_tests(self,title):
        ...
        self.CONF="%s/lib/conf.py"%os.getcwd()
